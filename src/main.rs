...
use conrod::{color, Button, Canvas, Circle, Color, Colorable, DropDownList, EnvelopeEditor,
             Frameable, Labelable, NumberDialer, Point, Positionable, Scrollbar, Slider, Sizeable,
             Text, TextBox, Theme, Toggle, Widget, WidgetMatrix, XYPad};
...
fn main() {
    let file = File::open("tests/documents/Schrodingers_cat.svg").unwrap();
    ...
    let mut window: PistonWindow = WindowSettings::new("All The Widgets!", [1100, 560])
                                       .opengl(opengl)
                                       .exit_on_esc(true)
                                       .vsync(true)
                                       .build()
...
fn set_widgets(ui: &mut UiCell, app: &mut DemoApp) {
    ...
    for i in 0..2usize {
        ...
        let env_y_max = if i == 0 {
            20_000.0
        } else {
            1.0
        };
        let env_skew_y = if i == 0 {
            3.0
        } else {
            1.0
        };
